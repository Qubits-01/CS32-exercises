...
#define arrSize 1000  // 8
#define prime 524287  // 13
#define newLine printf("\n")
...
void displayArr(int[]);
void displayArrWithRange(int[], int, int);
...
void shuffleArr(int newArr[], int oldArr[]) {
    int newIndices[arrSize];
    for (int i = 0; i < arrSize; i++) newIndices[i] = ((i + 1) * prime) % arrSize;
    for (int i = 0; i < arrSize; i++) newArr[i] = oldArr[newIndices[i]];
}
...
void displayArrWithRange(int myArr[], int left, int right) {
    for (;left < right; left++) printf("%d ", myArr[left]);
    newLine;
}
